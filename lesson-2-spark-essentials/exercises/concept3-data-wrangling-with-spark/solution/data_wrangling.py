...
spark = SparkSession \
    .builder \
    .config("spark.driver.host", "192.168.0.3") \
...
# # Data Exploration 
# user_log_df.show(truncate=False)
...
get_hour = udf(lambda x: datetime.datetime.fromtimestamp(x / 1000.0).month)
...
print(
user_log_valid_df.select(["userId","gender"]) \
.dropDuplicates() .groupby(user_log_valid_df.gender).count().show()
)
...
user_log_valid_df = user_log_valid_df \
    ...
    .over(windowval))
#
# user_log_valid_df.where(user_log_valid_df.downgraded !=0).show()
#
